...
struct Edge {
  ...
  void setMax() { this->w = this->wMax; }

  void setMin() { this->w = this->wMin; }
};
...
struct Graph {
  ...
  void newVentEdge(int from, int to, int wMin, int wMax,
                   std::vector<std::shared_ptr<Edge>> &v) {
    std::shared_ptr<Edge> newEdge = std::make_shared<Edge>();
    v.push_back(newEdge);
    newEdge->from = this->nodes[from];
    newEdge->to = this->nodes[to];
  ...
  std::pair<int, int> solve(int m, int s, int f,
    ...
    std::priority_queue<
        ...
        std::greater<std::pair<int, std::pair<std::shared_ptr<Node>, bool>>>>
        _pQueue;
    ...
    while (!_pQueue.empty()) {
      ...
      for (int i = 0; i < currNode->simpleAdj.size(); i++) {
        ...
        if (myShortestPath[testingNode->v] >
            myShortestPath[currNode->v] + testingNodeWeight) {
          if (isSus)
            prevEdge[testingNode->v] = currNode->simpleAdj[i];
          myShortestPath[testingNode->v] =
              myShortestPath[currNode->v] + testingNodeWeight;
          _pQueue.push(std::make_pair(myShortestPath[testingNode->v],
                                      std::make_pair(testingNode, isSus)));
        }
      }
      ...
      for (int i = 0; i < currNode->ventAdj.size(); i++) {
        ...
        if (currNode->ventAdj[i]->w == 0)
          isSus ? currNode->ventAdj[i]->setMin()
                : currNode->ventAdj[i]->setMax();
    ...
    std::pair<int, int> res =
        std::make_pair(shortestPathMontry[f], shortestPathStud[f]);
