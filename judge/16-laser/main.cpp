...
struct Graph{
  ...
  int solve(std::stack<std::shared_ptr<Node> >& _stack){
    ...
    std::priority_queue<
      ...
      std::greater<std::pair<int, std::shared_ptr<Node> > > > _pQueue;
    ...
    while(!_pQueue.empty()){
      ...
      for(int i = 0; i < currNode->adj.size(); i++){
        ...
        std::shared_ptr<Node> testingNode = (testingEdge->n1 == currNode) ? testingEdge->n2 : testingEdge->n1;
        ...
        if((normalizedTimeToHere % tCycle) >= testingEdge->tOff || ((normalizedTimeToHere + testingNodeWeight) % tCycle) > testingEdge->tOff)
        ...
        if(shortestPath[testingNode->v] > timeToHere + testingNodeWeight){
          ...
          if(shortestPath[testingNode->v] <= 1000000)
            _pQueue.push(std::make_pair(shortestPath[testingNode->v], testingNode));
        } 
      }
    }
